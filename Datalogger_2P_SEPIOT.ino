...
void setup(void) {
  ...
  Serial.println(F("Configurando: Tareas Locales"));
  TSK.inicializar_tareas();
  Serial.println(F("Configurando: Reloj DS1307"));
  MIRTC.RTC_init();
  Serial.println(F("Configurando: Sensores"));
  SEN.config();
  Serial.println(F("Configurando: Actuadores"));
  ACT.config();
  Serial.println(F("Configurando: MicroSD"));
  MSD.MicroSD_init();
  Serial.println(F("Configurando: WiFi"));
  mqtt.setup_WiFi();
  Serial.println(F("Configurando: Servidor MQTT"));
  mqtt.set_MQTT_server();
  Serial.println(F("Configurando: Receptor MQTT"));
